...
(
    ...
    CONSTRAINT pk_trabaja PRIMARY KEY (ID),
    CONSTRAINT ck_trabaja UNIQUE (IDTrabajador, IDEdicion)
);
...
(
    ...
    CONSTRAINT pk_asignado PRIMARY KEY (ID),
    CONSTRAINT ck_asignado UNIQUE (IDTrabajador, IDEdicion, FechaFin, FechaIni, IDPista)
);
...
CONSTRAINT pk_ten_ed_entre PRIMARY KEY(ID),
CONSTRAINT ck_ten_ed_entre UNIQUE (IDEdicion, IDTenista)
);
...
CONSTRAINT ck_partido UNIQUE (IDPista, Fecha)
);
...
CONSTRAINT pk_compite PRIMARY KEY(ID),
CONSTRAINT ck_compite UNIQUE(IDPista, Fecha, IDEdicion, IDTenista)
);
...
IDCompra int REFERENCES Compra(IDCompra) ,
Cantidad int,
CONSTRAINT pk_compra_entr PRIMARY KEY(ID),
CONSTRAINT ck_compra_entr UNIQUE(IDTipo, IDCompra)
);
